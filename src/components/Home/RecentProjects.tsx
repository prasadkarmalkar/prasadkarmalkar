...
function RecentProjects() {
	return (
		<section className='my-20 m-auto'>
			<h2 className='text-center text-3xl sm:text-4xl'>Recent <strong><GradientText>Creations</GradientText></strong></h2>
						...
						<div key={project.name} className='relative'>
							<div className="overflow-hidden rounded-3xl relative">
								...
								<div className='bg-gradient-to-t absolute bottom-0 py-4 from-gray-950 to-transparent w-full'>
									<h3 className='text-xl font-semibold sm:text-2xl px-4'>{project.name}</h3>
								</div>
							</div>
			...
			<div className='m-auto text-center mt-16'>
				<Link href={'/projects'} className='border rounded-full px-4 py-2 hover:text-black hover:bg-white transition-all duration-300'>Load More</Link>
			</div>
		</section>
	);
}
